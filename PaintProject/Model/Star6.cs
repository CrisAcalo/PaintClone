...
namespace PaintProject.Model
{
    public class Star6
    {
        ...
        public Star6(PictureBoxCanvas canvas)
        {
            this.canvas = canvas;
        }
        ...
        public void MouseUp(MouseEventArgs e)
        {
            ...
            {
                isDrawing = false;
                DrawStar6Point(startPoint, e.Location);
            }
        }

        private void DrawStar6Point(Point start, Point end)
        {
            // Calcular el tamaño y posición de la estrella
            int radius = (int)Math.Sqrt(Math.Pow(end.X - start.X, 2) + Math.Pow(end.Y - start.Y, 2));
            Point[] starPoints = new Point[12];
            double angle = 2 * Math.PI / 12; // 30 grados en radianes
